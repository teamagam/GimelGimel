...
public abstract class AbstractJtsCodec<S, T extends Geometry<?>> implements Codec<S, T> {
  ...
  protected com.vividsolutions.jts.geom.Geometry toJtsGeometryCollection(Geometry<?> src) {
    ...
    if (src instanceof Point) {
      ...
      AbstractJtsCodec<com.vividsolutions.jts.geom.MultiPoint, MultiPoint> codec =
          new MultiPointCodec(this.geometryFactory);
      ...
      AbstractJtsCodec<com.vividsolutions.jts.geom.MultiLineString, MultiLineString> codec =
          new MultiLineStringCodec(this.geometryFactory);
      com.vividsolutions.jts.geom.MultiLineString multiLineString =
          codec.fromGeometry((MultiLineString) src);
      returnGeometry = multiLineString;
    } else if (src instanceof MultiPolygon) {
      AbstractJtsCodec<com.vividsolutions.jts.geom.MultiPolygon, MultiPolygon> codec =
          new MultiPolygonCodec(this.geometryFactory);
      com.vividsolutions.jts.geom.MultiPolygon multiPolygon =
          codec.fromGeometry((MultiPolygon) src);
      returnGeometry = multiPolygon;
    } else if (src instanceof GeometryCollection) {
      ArrayList<com.vividsolutions.jts.geom.Geometry> geometries =
      ...
      for (Geometry<?> geometry : geometryCollection.getGeometries()) {
        com.vividsolutions.jts.geom.Geometry jtsGeometry =
            toJtsGeometryCollection(geometry); // recursion!
        geometries.add(jtsGeometry);
      }
      returnGeometry = this.geometryFactory.createGeometryCollection(
          geometries.toArray(new com.vividsolutions.jts.geom.Geometry[geometryCollection.size()]));
    } else {
  ...
  protected Geometry<?> fromJtsGeometryCollection(com.vividsolutions.jts.geom.Geometry src) {
    ...
    if (src.getGeometryType().equalsIgnoreCase(Geometry.Type.GEOMETRY_COLLECTION.getValue())) {
      ...
      MultiLineString multiLineString =
          codec.toGeometry((com.vividsolutions.jts.geom.MultiLineString) src);
      returnGeometry = multiLineString;
      ...
      throw new IllegalArgumentException(
          "Unsupported geometry type: " + src.getGeometryType()); //$NON-NLS-1$
    }
  ...
  protected com.vividsolutions.jts.geom.Polygon toJtsPolygon(Polygon src) {
    ...
    return this.geometryFactory.createPolygon(toJtsLinearRing(src.perimeter()),
  ...
  protected com.vividsolutions.jts.geom.LinearRing toJtsLinearRing(LinearRing src) {
    ...
    return this.geometryFactory.createLinearRing(FluentIterable.from(src.positions().children())
  ...
  protected com.vividsolutions.jts.geom.LineString toJtsLineString(LineString src) {
    ...
    return this.geometryFactory.createLineString(FluentIterable.from(src.positions().children())
        .transform(SinglePosition.coordinatesFn())
        .transform(toJtsCoordinateFn())
        .toArray(Coordinate.class)

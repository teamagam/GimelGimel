...
public class FilesDownloader {
  ...
  private static void createParentDirectoryIfNecessary(File targetFile) {
    ...
    if (!parentFile.mkdirs() && !parentFile.isDirectory()) {
      throw new RuntimeException("Couldn't create parent directory for " + targetFile.getPath());
    }
  }
  ...
  private InputStream getDownloadStream(URL downloadURL) throws IOException {
    return mFilesDownloaderAPI.downloadFile(downloadURL.toString()).execute().body().byteStream();
  }

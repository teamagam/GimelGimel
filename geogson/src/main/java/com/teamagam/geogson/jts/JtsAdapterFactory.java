...
import com.teamagam.geogson.core.codec.CodecRegistry;
 ...
 *         .registerTypeAdapterFactory(new JtsAdapterFactory(new GeometryFactory(new
 * PrecisionModel(100), 4326)))
...
public class JtsAdapterFactory implements TypeAdapterFactory {
   ...
   *
   * @param geometryFactory an optional {@link GeometryFactory} defining a PrecisionModel
   * and a SRID
   */
...
class JtsGeometryAdapter extends TypeAdapter<com.vividsolutions.jts.geom.Geometry> {
  ...
  public void write(JsonWriter out, com.vividsolutions.jts.geom.Geometry value) throws IOException {
    ...
    gson.getAdapter(new TypeToken<Geometry<?>>() {
    }).write(out, to);
  }
  ...
  public com.vividsolutions.jts.geom.Geometry read(JsonReader in) throws IOException {
    ...
    Geometry<?> geometry = gson.getAdapter(new TypeToken<Geometry<?>>() {
    }).read(in);

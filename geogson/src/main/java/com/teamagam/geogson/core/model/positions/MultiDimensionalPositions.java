...
package com.teamagam.geogson.core.model.positions;

import com.google.common.collect.ImmutableList;

/**
 * It represent a collection of {@link AreaPositions} used to represent composed Are geometries like
 * the
...
public class MultiDimensionalPositions extends AbstractPositions<AreaPositions> {
   ...
   * {@link com.github.filosganga.geogson.model.Polygon} with no holes. This is probably not
   * compliant with the
   * GeoJson specifications, but it seems to happen quite frequently. It will be handled as the
   * AreaPositions case.
   * - AreaPositions, it will return a new MultiDimensionalPositions by appending the given
   * AreaPositions to this.
  ...
  public Positions merge(Positions other) {
    if (other instanceof SinglePosition) {
      ...
      throw new IllegalArgumentException(
          "Cannot merge single position and multidimensional positions");
      ...
      return new MultiDimensionalPositions(
          ImmutableList.<AreaPositions>builder().addAll(children).add(that).build());
    } else {

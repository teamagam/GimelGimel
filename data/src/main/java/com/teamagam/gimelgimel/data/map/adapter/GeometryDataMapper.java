...
public class GeometryDataMapper {
  ...
  public Polygon transform(com.teamagam.geogson.core.model.Polygon geoData) {
    ...
    if (geoData != null) {
      List<PointGeometry> pointGeometries = extractPointGeometries(geoData.positions().children());
  ...
  public Geometry transformBySuperclass(com.teamagam.geogson.core.model.Geometry geoData) {
  ...
  public com.teamagam.geogson.core.model.Geometry transformToDataBySuperclass(Geometry geometry) {
  ...
  private PointGeometry createPointGeometry(double lat, double lon, double alt) {
    return Double.isNaN(alt) ? new PointGeometry(lat, lon) : new PointGeometry(lat, lon, alt);
  }
  ...
  private class GeometryToDataTransformer implements IGeometryVisitor {
    ...
    private List<SinglePosition> getSinglePositions(List<PointGeometry> points) {
      ...
      for (PointGeometry point : points) {
        coordinates.add(
            new SinglePosition(Coordinates.of(point.getLongitude(), point.getLatitude())));
      }

      return coordinates;
    }

    private Iterable<LinearPositions> getIterableLinearPositions(List<SinglePosition> coordinates) {
